...
export function makeCloudflareWorkerFetcher() {
  const fetcher: Fetcher = async (url, ops) => {
    const { workerRequest } = await import(
      "@/backend/cloudflareWorker/cloudflareWorker"
    );

    const result = await workerRequest<any>({
      url,
      method: ops.method,
      headers: ops.headers,
      body: convertBodyToObject(ops.body),
